...
<template>
  <view :class="rootClass" aria-labelledby="profile-edit-title">
    <view class="mb-5 grid gap-1">
      <text class="text-xs font-semibold uppercase tracking-[0.16em] text-teal-700">Profile</text>
      <text id="profile-edit-title" class="text-xl font-bold text-slate-950">{{ title }}</text>
    </view>
    ...
    <form class="grid gap-4" @submit="submit">
      ...
      <VInput
        ...
        placeholder="请输入手机号"
      />
      <view class="grid gap-2 text-sm font-semibold text-slate-700">
        <text>所在城市</text>
