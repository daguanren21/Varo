...
function headingClass(level?: number) {
  if (level === 1) {
    return 'mt-4 mb-2 text-[22px] font-black leading-tight text-inherit'
  }
  if (level === 2) {
    return 'mt-3.5 mb-2 text-[19px] font-black leading-tight text-inherit'
  }
  if (level === 3) {
    return 'mt-3 mb-1.5 text-[16px] font-extrabold leading-snug text-inherit'
  }
...
<template>
  ...
  <view v-else-if="node.kind === 'list'" class="my-2 grid gap-1.5">
    <view v-for="(child, index) in node.children" :key="index" class="flex items-start gap-2 pl-1">
      ...
      <view class="min-w-0 flex-1">
        <AgentMarkdownNode :node="child" @link="emit('link', $event)" />
      </view>
    </view>
  </view>
  ...
  <view v-else-if="node.kind === 'code-block'" class="my-2 overflow-hidden rounded-[14px] border border-slate-800 bg-slate-950 text-slate-200">
    <view class="flex min-h-8 items-center border-b border-slate-800 bg-slate-900 px-3 text-[10px] font-bold uppercase tracking-widest text-slate-400">
      {{ node.language || 'text' }}
    </view>
    <scroll-view class="max-w-full" scroll-x>
      <text class="block whitespace-pre px-3 py-2.5 font-mono text-[11px] leading-5">
        {{ node.text }}
      </text>
    </scroll-view>
  </view>
  ...
  <view v-else-if="node.kind === 'emphasis'" class="inline italic">
    <AgentMarkdownNode v-for="(child, index) in node.children" :key="index" :node="child" @link="emit('link', $event)" />
  </view>
  <view v-else-if="node.kind === 'strikethrough'" class="inline line-through">
    <AgentMarkdownNode v-for="(child, index) in node.children" :key="index" :node="child" @link="emit('link', $event)" />
  </view>
  <view v-else-if="node.kind === 'highlight'" class="inline rounded bg-amber-100 px-0.5">
    <AgentMarkdownNode v-for="(child, index) in node.children" :key="index" :node="child" @link="emit('link', $event)" />
  </view>
  <view v-else-if="node.kind === 'insert'" class="inline underline">
    <AgentMarkdownNode v-for="(child, index) in node.children" :key="index" :node="child" @link="emit('link', $event)" />
  </view>
  <view v-else-if="node.kind === 'subscript' || node.kind === 'superscript'" class="inline text-[10px]">
    <AgentMarkdownNode v-for="(child, index) in node.children" :key="index" :node="child" @link="emit('link', $event)" />
  </view>
  ...
  <view v-else-if="node.kind === 'math-block'" class="my-2 overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 p-3 font-mono text-xs">
    {{ node.text }}
  </view>

  <view v-else-if="node.kind === 'admonition'" class="my-2 rounded-xl border border-teal-200 bg-teal-50 p-3">
    <text v-if="node.title" class="mb-1 block font-bold text-teal-900">
      {{ node.title }}
    </text>
    <AgentMarkdownNode v-for="(child, index) in node.children" :key="index" :node="child" @link="emit('link', $event)" />
  </view>
