...
const rootClass = computed(() =>
  cn('agent-file-diff overflow-hidden rounded-[14px] border border-slate-200 bg-white shadow-sm', props.className)
)
  ...
  >
    <view class="agent-file-diff__header">
      <view class="agent-file-diff__identity">
        ...
        <view class="agent-file-diff__path">
          <text v-if="path.directory" class="agent-file-diff__directory">{{ path.directory }}</text>
          <text class="agent-file-diff__basename">{{ path.basename }}</text>
        </view>
        <view class="agent-advanced__status inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-500" :data-status="status">
      ...
      <view class="agent-file-diff__header-actions">
        <view class="agent-file-diff__counts">
          <text data-kind="addition" :aria-label="`${counts.additions} ${resolvedLabels.additions}`">+{{ counts.additions }}</text>
          <text data-kind="deletion" :aria-label="`${counts.deletions} ${resolvedLabels.deletions}`">−{{ counts.deletions }}</text>
        </view>
        <button
    ...
    <view v-if="currentOpen" :id="bodyId" class="agent-file-diff__body" role="region" :aria-label="filename">
      ...
      <scroll-view class="agent-file-diff__viewport" scroll-x :show-scrollbar="false">
        <view v-if="!lines.length" class="agent-file-diff__empty">{{ resolvedLabels.empty }}</view>
        ...
        <view v-else-if="currentView === 'unified'" class="agent-file-diff__unified">
          <template v-for="(line, index) in lines" :key="line.id ?? index">
            ...
            >
              ...
              <text class="agent-file-diff__hunk-label">{{ line.content }}</text>
              <text v-if="line.collapsedLines" class="agent-file-diff__hunk-count">{{ line.collapsedLines }} {{ resolvedLabels.unchanged }}</text>
            </button>
            <button
              v-else
            ...
            >
              <text v-if="currentLineNumbers" class="agent-file-diff__number" data-side="old" aria-hidden="true">{{ line.oldNumber ?? '' }}</text>
              <text v-if="currentLineNumbers" class="agent-file-diff__number" data-side="new" aria-hidden="true">{{ line.newNumber ?? '' }}</text>
              <text class="agent-file-diff__marker" aria-hidden="true">{{ agentDiffMarker(line.type) }}</text>
              <slot name="line" :index="index" :line="line" side="unified">
                <text class="agent-file-diff__code">
                  <text>{{ inlineSegments[index][0] }}</text>
                  <text v-if="inlineSegments[index][1]" class="agent-file-diff__inline-change">{{ inlineSegments[index][1] }}</text>
        ...
        <view v-else class="agent-file-diff__split">
          <template v-for="(row, rowIndex) in splitRows" :key="row.hunk?.line.id ?? `row:${rowIndex}`">
            ...
            >
              <text class="agent-file-diff__hunk-icon" aria-hidden="true" />
              <text class="agent-file-diff__hunk-label">{{ row.hunk.line.content }}</text>
              <text v-if="row.hunk.line.collapsedLines" class="agent-file-diff__hunk-count">{{ row.hunk.line.collapsedLines }} {{ resolvedLabels.unchanged }}</text>
            </button>
            <view v-else class="agent-file-diff__split-row">
              ...
              >
                <text v-if="currentLineNumbers" class="agent-file-diff__number" data-side="old" aria-hidden="true">{{ row.deletion.line.oldNumber ?? '' }}</text>
                <text class="agent-file-diff__marker" aria-hidden="true">{{ agentDiffMarker(row.deletion.line.type) }}</text>
                <slot name="line" :index="row.deletion.index" :line="row.deletion.line" side="old">
                  <text class="agent-file-diff__code">
                    <text>{{ inlineSegments[row.deletion.index][0] }}</text>
                    <text v-if="inlineSegments[row.deletion.index][1]" class="agent-file-diff__inline-change">{{ inlineSegments[row.deletion.index][1] }}</text>
              ...
              >
                <text v-if="currentLineNumbers" class="agent-file-diff__number" data-side="new" aria-hidden="true">{{ row.addition.line.newNumber ?? '' }}</text>
                <text class="agent-file-diff__marker" aria-hidden="true">{{ agentDiffMarker(row.addition.line.type) }}</text>
                <slot name="line" :index="row.addition.index" :line="row.addition.line" side="new">
                  <text class="agent-file-diff__code">
                    <text>{{ inlineSegments[row.addition.index][0] }}</text>
                    <text v-if="inlineSegments[row.addition.index][1]" class="agent-file-diff__inline-change">{{ inlineSegments[row.addition.index][1] }}</text>
    ...
    <view v-if="currentOpen && showActions" class="agent-file-diff__footer">
      <text><text class="agent-file-diff__changed-count">{{ counts.additions + counts.deletions }}</text> {{ resolvedLabels.changed }}</text>
      <view>
        <button class="agent-file-diff__action" type="button" :disabled="disabled" @click="emit('reject')">{{ resolvedLabels.reject }}</button>
        <button class="agent-file-diff__action" type="button" :disabled="disabled" @click="emit('accept')">{{ resolvedLabels.accept }}</button>
      </view>
    </view>
  </view>
</template>
...
<style scoped>
.agent-file-diff__directory { flex: 0 1 auto; color: var(--agent-diff-muted); font-size: 10px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.agent-file-diff__basename { flex: 0 1 auto; color: var(--agent-diff-text); font-size: 12px; font-weight: 780; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.agent-file-diff__changed-count { color: var(--agent-diff-text); font-weight: 700; }
.agent-file-diff__split { display: block; width: 760px; min-width: 760px; }
.agent-file-diff__split-row { display: grid; grid-template-columns: 380px 380px; }
.agent-file-diff[data-wrap='true'] .agent-file-diff__split { width: 640px; min-width: 640px; }
.agent-file-diff[data-wrap='true'] .agent-file-diff__split-row { grid-template-columns: 320px 320px; }
.agent-file-diff__hunk:disabled { opacity: 1; }
@media (max-width: 600px) { .agent-file-diff__directory { display: none; } }
</style>
