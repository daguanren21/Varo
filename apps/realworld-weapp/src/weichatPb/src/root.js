...
function parseFromPbString(pbString, options, callback) {
  ...
  const name = pbObj['name']
  const pbJsonStr = pbObj['pbJsonStr']
  ...
  function fetch(name) {
    if (self.names.includes(name)) { return }
...
Root.prototype.load = function load(filename, options, callback) {
  ...
  function fetch(filename, weak) {
    ...
    if (self.files.includes(filename)) { return }
    ...
    if (sync) {
      ...
      ++queued
      util.fetch(filename, (err, source) => {
        --queued
        /* istanbul ignore if */
...
Root.prototype.loadSync = function loadSync(filename, options) {
  if (!util.isNode) { throw new Error('not supported') }
...
Root.prototype.resolveAll = function resolveAll() {
  if (this.deferred.length) {
    throw new Error(`unresolvable extensions: ${this.deferred.map((field) => {
...
function tryHandleExtension(root, field) {
  ...
  if (extendedType) {
    const sisterField = new Field(field.fullName, field.id, field.type, field.rule, undefined, field.options)
...
Root.prototype._handleAdd = function _handleAdd(object) {
  if (object instanceof Field) {
    ...
    for (let j = 0; j < /* initializes */ object.nestedArray.length; ++j) // recurse into the namespace
...
Root.prototype._handleRemove = function _handleRemove(object) {
  if (object instanceof Field) {
    ...
    for (let i = 0; i < /* initializes */ object.nestedArray.length; ++i) // recurse into the namespace
