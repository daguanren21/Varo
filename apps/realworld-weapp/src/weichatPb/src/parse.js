...
const base10Re = /^[1-9]\d*$/
const base10NegRe = /^-?[1-9]\d*$/
const base16Re = /^0x[0-9a-fA-F]+$/
const base16NegRe = /^-?0x[0-9a-fA-F]+$/
const base8Re = /^0[0-7]+$/
const base8NegRe = /^-?0[0-7]+$/
const numberRe = /^(?!e)\d*(?:\.\d*)?(?:e[+-]?\d+)?$/i
const nameRe = /^[a-z_]\w*$/i
const typeRefRe = /^(?:\.?[a-z_]\w*)+$/i
const fqTypeRefRe = /^(?:\.[a-z]\w*)+$/i
...
function parse(source, root, options) {
  ...
  function illegal(token, name, insideTryCatch) {
    ...
    return new Error(`illegal ${name || 'token'} '${token}' (${filename ? `${filename}, ` : ''}line ${tn.line})`)
  }
  ...
  function parseNumber(token, insideTryCatch) {
    ...
    if (base16Re.test(token)) { return sign * Number.parseInt(token, 16) }
    if (base8Re.test(token)) { return sign * Number.parseInt(token, 8) }

    /* istanbul ignore else */
    if (numberRe.test(token)) { return sign * Number.parseFloat(token) }
  ...
  function parseId(token, acceptNegative) {
    ...
    if (base16NegRe.test(token)) { return Number.parseInt(token, 16) }

    /* istanbul ignore else */
    if (base8NegRe.test(token)) { return Number.parseInt(token, 8) }
  ...
  function parseMethod(parent, token) {
    ...
    let responseType; let responseStream
