...
function valuePartial_fromObject(field, fieldIndex, propName, options) {
  ...
  const ksi = options['ksi']
  const ksiFlag = typeof ksi != 'undefined'
  if (field.resolvedType) {
    if (field.resolvedType instanceof Enum) {
      const prop = ksiFlag ? d[propName][ksi] : d[propName]
      ...
      if (typeof (ksiFlag ? d[propName][ksi] : d[propName]) !== 'object') { throw new TypeError(`${field.fullName}: object expected`) }
      ksiFlag
  ...
  return function (options) {
    return function (d) {
      ...
      for (let i = 0; i < fields.length; ++i) {
        ...
        if (field.map) {
          if (d[propName]) {
            if (typeof d[propName] !== 'object') { throw new TypeError(`${field.fullName}: object expected`) }
            m[propName] = {}
          }
          ...
          if (d[propName]) {
            if (!Array.isArray(d[propName])) { throw new TypeError(`${field.fullName}: array expected`) }
            m[propName] = []
...
function valuePartial_toObject(field, fieldIndex, propName, options) {
  ...
  if (field.resolvedType) {
    ...
    switch (field.type) {
      ...
      case 'float':
        ksiFlag
          ? (d[propName][ksi] = o.json && !isFinite(m[propName][ksi]) ? String(m[propName][ksi]) : m[propName][ksi])
        ...
        if (o.defaults) {
          for (i = 0; i < normalFields.length; ++i) {
            ...
            if (field.resolvedType instanceof Enum) {
              ...
              if (util.Long) {
                const n = new util.Long(field.typeDefault.low, field.typeDefault.high, field.typeDefault.unsigned)
        ...
        if (field.map) {
          ...
          if (m[propName] && (ks2 = Object.keys(m[propName]).length)) {
            ...
            for (j = 0; j < ks2.length; ++j) {
              valuePartial_toObject(field, index, propName, util.merge(util.copy(options), { m, d, ksi: ks2[j], o }))
            }
          }
        }
        else if (field.repeated) {
          if (m[propName] && m[propName].length) {
            ...
            for (j = 0; j < m[propName].length; ++j) {
              valuePartial_toObject(field, index, propName, util.merge(util.copy(options), { m, d, ksi: j, o }))
            }
          }
        }
        else {
          if (m[propName] != null && (Object.hasOwn(m, propName)/* || field.partOf */)) {
