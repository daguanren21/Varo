 ...
 * @param {number} id Unique id within its namespace
 * @param {string} type Value type
 * @param {string|Object.<string,*>} [rule] Field rule
 * @param {string|Object.<string,*>} [extend] Extended type if different from parent
 * @param {Object.<string,*>} [options] Declared options
...
function Field(name, id, type, rule, extend, options, comment) {
  ...
  if (!util.isInteger(id) || id < 0) { throw new TypeError('id must be a non-negative integer') }

  if (!util.isString(type)) { throw new TypeError('type must be a string') }

  if (rule !== undefined && !ruleRe.test(rule = rule.toString().toLowerCase())) { throw new TypeError('rule must be a string rule') }

  if (extend !== undefined && !util.isString(extend)) { throw new TypeError('extend must be a string') }
 ...
 * @param {"optional"|"required"|"repeated"} [fieldRule] Field rule
