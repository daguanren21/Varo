...
function verifyValue(field, fieldIndex, ref, options) {
  ...
  if (field.resolvedType) {
    if (field.resolvedType instanceof Enum) {
      ...
      const e = _types[fieldIndex].verify(ref)
...
function verifier(mtype) {
  return function (options) {
    return function (m) {
      ...
      let p
      if (oneofs.length) { p = {} }
      for (let i = 0; i < mtype.fieldsArray.length; ++i) {
        const field = mtype._fieldsArray[i].resolve()
        const ref = m[field.name]
        if (!field.optional || (ref != null && Object.hasOwn(m, field.name))) {
          var _i
          if (field.map) {
            ...
            if (field.partOf) {
              const oneofPropName = field.partOf.name
              if (seenFirstField[field.partOf.name] === 1) {
                if (p[oneofPropName] === 1) { return `${field.partOf.name}: multiple values` }
              }
              p[oneofPropName] = 1
            }
