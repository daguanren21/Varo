...
function OneOf(name, fieldNames, options, comment) {
  ...
  /* istanbul ignore if */
  if (!(fieldNames === undefined || Array.isArray(fieldNames))) { throw new TypeError('fieldNames must be an Array') }
...
OneOf.prototype.add = function add(field) {
  /* istanbul ignore if */
  if (!(field instanceof Field)) { throw new TypeError('field must be a Field') }
...
OneOf.prototype.remove = function remove(field) {
  ...
  /* istanbul ignore if */
  if (index < 0) { throw new Error(`${field} is not a member of ${this}`) }
...
OneOf.d = function decorateOneOf() {
  const fieldNames = Array.from({ length: arguments.length })
  let index = 0
