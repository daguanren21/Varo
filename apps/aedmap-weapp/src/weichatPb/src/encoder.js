  ...
  return function (options) {
    ...
    return function (message, writer) {
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        const type = field.resolvedType instanceof Enum ? 'uint32' : field.type
        const wireType = types.basic[type]
        let ref = message[field.name]
        ...
        // 正式进行序列化
        if (field.map) { // 有待验证
          if (ref != null && Object.hasOwn(message, field.name)) {
            for (let ks = Object.keys(ref), l = 0; l < ks.length; ++l) {
          ...
          if (!field.optional || (ref != null && (Object.hasOwn(message, field.name) /* || field.partOf */))) {
            if (!field.optional && (ref == null || !Object.hasOwn(message, field.name))) {
