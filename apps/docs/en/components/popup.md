...
<RegistryInstallStrip item="components/popup" :targets="['h5', 'weapp']" locale="en" />
...
`closeIcon` accepts a supported built-in `VIcon` name such as `close`, `back`, `check`, or `chevron-left`; it defaults to `close`. Use the `#closeIcon` slot for a custom icon component or custom graphic content.

```vue
<template>
  <VPopup v-model:visible="visible" closeable close-icon="back" />

  <VPopup v-model:visible="customVisible" closeable>
    <template #closeIcon>
      <VIcon name="check" />
    </template>
  </VPopup>
...
| `closeIcon`           | `string`                                                       | `'close'`     | Supported built-in `VIcon` name         |
