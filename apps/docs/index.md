...
Registry 同时支持 `h5` 与 `weapp`：H5 覆盖 56 个 runtime 组件族；小程序开放 45 个高共识组件族，并增加 `RegionPicker` 与原生 `Map` 扩展。copy-owned Weapp renderer 均为 target-specific 原生 Wevu SFC；纯 adapter 可重导出目标 primitives，双端仅共享类型、纯函数和 headless primitives。完整分级由 `registry/component-tiers.v0.1.json` 约束。
