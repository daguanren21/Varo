...
<template>
  <section class="agent-component-demo" :data-demo="component">
    ...
    <nav class="agent-component-demo__tabs" aria-label="Demo views">
      <button type="button" :data-active="String(demoTab === 'preview')" @click="demoTab = 'preview'">Preview</button>
      <button type="button" :data-active="String(demoTab === 'code')" @click="demoTab = 'code'">Code</button>
    </nav>
    ...
    <div v-show="demoTab === 'preview'" class="agent-component-demo__stage">
      ...
      <div v-else-if="component === 'message'" class="agent-component-demo__stack"><AgentMessage role="assistant" label="Varo Agent">已完成组件审计。</AgentMessage><AgentMessage role="user" label="你">继续生成 API 文档。</AgentMessage></div>
      <AgentConversation v-else-if="component === 'conversation'" :messages="messages" />
      <div v-else-if="component === 'tool-chip'" class="agent-component-demo__row"><AgentToolChip v-for="tool in tools" :key="tool.id" :tool="tool" /></div>
      ...
      <AgentMessageScroller v-else-if="component === 'message-scroller'" :at-live-edge="false" @follow="done('跳到最新消息')"><AgentConversation :messages="messages" /></AgentMessageScroller>
      ...
      <AgentToolApproval v-else-if="component === 'tool-approval'" tool="npm.publish" description="允许 Agent 发布 @varo/agent-core？" :details="[{ label: 'Package', value: '@varo/agent-core' }, { label: 'Tag', value: 'latest' }]" @allow="done('允许一次')" @deny="done('已拒绝')" />
...
.agent-component-demo__stage {
  ...
  background:
    ...
    linear-gradient(90deg, var(--varo-gridline) 1px, transparent 1px),
    var(--varo-demo-surface);
...
.agent-component-demo__stage > :deep(:first-child) {
  animation: varo-agent-demo-enter 320ms cubic-bezier(.2, .8, .2, 1) both;
}
...
:global(.dark) .agent-component-demo__stage :deep(.bg-slate-50\/90),
...
:global(.dark) .agent-component-demo__stage :deep(
