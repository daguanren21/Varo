...
describe('docs navigation', () => {
  ...
  it('documents shadcn-style install and secondary business wrapping', () => {
    ...
    expect(shadcnZh).toContain('Varo 只有一套 shadcn 模式')
    expect(shadcnZh).toContain('src/components/ui/form.ts')
    expect(shadcnZh).toContain('`v-form.vue` 与 `v-form-item.vue`')
    expect(shadcnZh).toContain('Reka UI 是 Radix Vue 的后续版本')
    expect(shadcnZh).toContain('`vee-validate` 直接依赖并导入 Vue')
    expect(shadcnZh).toContain('`useForm`、`useField` 和 `defineRule`')
    ...
    expect(shadcnEn).toContain('Varo has one shadcn mode')
    expect(shadcnEn).toContain('src/components/ui/form.ts')
    expect(shadcnEn).toContain('`v-form.vue` and `v-form-item.vue`')
    expect(shadcnEn).toContain('Reka UI is the successor to Radix Vue')
    expect(shadcnEn).toContain('`vee-validate` directly peers on and imports Vue')
    expect(shadcnEn).toContain('`useForm`, `useField`, and `defineRule`')
  })
