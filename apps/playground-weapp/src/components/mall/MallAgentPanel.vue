...
const approvalTitle = computed(() => {
  if (props.pendingAction?.type === 'purchase') return `确认购买 ${props.pendingAction.product.name}`
  if (props.pendingAction?.type === 'return') return `确认退货 ${props.pendingAction.order.productName}`
  return ''
})
  ...
  <view v-if="open" class="fixed bottom-0 left-0 right-0 top-0 z-50 overflow-hidden bg-slate-950/45" @click.self="emit('close')">
    <view class="absolute bottom-0 left-0 right-0 grid max-h-[92vh] min-h-[72vh] max-w-full grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-t-[24px] bg-slate-50 shadow-[0_-20px_60px_rgba(15,23,42,.24)]">
      <view class="box-border flex min-h-14 w-full min-w-0 items-center gap-3 overflow-hidden border-b border-slate-200 bg-white px-3.5">
        <view class="grid h-9 w-9 flex-none place-items-center rounded-xl bg-teal-700 text-sm font-black text-white">V</view>
        <view class="grid min-w-0 flex-1 gap-0.5">
          <view class="flex items-center gap-2">
            <text class="text-sm font-extrabold text-slate-950">Varo 购物 Agent</text>
            <text class="rounded-full bg-emerald-50 px-2 py-0.5 text-[9px] font-bold text-emerald-700">本地演示</text>
          </view>
          <text class="text-[10px] text-slate-400">购买与退货始终需要人工确认</text>
        </view>
