...
<template>
  <view :class="rootClass">
    <view class="h-28 w-28 overflow-hidden rounded-xl bg-slate-100">
      <VButton block size="sm" variant="ghost" tone="default" class-name="!h-28 !min-h-28 !w-28 !overflow-hidden !rounded-xl !bg-slate-100 !p-0" :aria-label="itemAriaLabel" @click="emit('select', item)">
    ...
    <view class="grid min-w-0 content-between gap-2">
      <view class="grid gap-1">
        <view class="flex min-w-0 items-start gap-2">
          <VButton block size="sm" variant="ghost" tone="default" class-name="!block !min-h-11 !min-w-0 !flex-1 !bg-transparent !px-2 !py-0 !text-left !text-sm !font-bold !leading-5 !text-slate-950" @click="emit('select', item)">
            {{ item.name }}
          </VButton>
