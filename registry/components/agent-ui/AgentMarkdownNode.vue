...
const richTextNodes = computed(() => toAgentRichTextNodes(props.node.children ?? []))
...
<template>
  ...
  <view v-else-if="node.kind === 'code-block'" class="my-2 overflow-hidden rounded-[14px] border border-slate-800 bg-slate-950 text-slate-200">
    <view class="flex min-h-8 items-center border-b border-slate-800 bg-slate-900 px-3 text-[11px] font-bold uppercase tracking-widest text-[var(--varo-agent-muted)]">
      {{ node.language || 'text' }}
    </view>
    <scroll-view class="max-w-full" scroll-x>
      <text class="block whitespace-pre px-3 py-2.5 font-mono text-[12px] leading-5">
        {{ node.text }}
      </text>
    </scroll-view>
  </view>
  ...
  <view v-else-if="node.kind === 'math-block'" class="my-2 overflow-x-auto rounded-xl border border-[var(--varo-agent-border)] bg-[var(--varo-agent-surface-strong)] p-3 font-mono text-xs">
    {{ node.text }}
  </view>

  <view v-else-if="node.kind === 'admonition'" class="my-2 rounded-xl border border-[var(--varo-agent-border-strong)] bg-[var(--varo-agent-primary-soft)] p-3">
