...
<template>
  <view class="grid min-h-[72vh] grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-[24px] border border-slate-200 bg-slate-50 shadow-[0_20px_60px_rgba(15,23,42,.12)]" aria-label="Agent conversation">
    <view class="flex min-h-16 items-center gap-3 border-b border-slate-200 bg-white px-4">
      <text class="grid h-10 w-10 flex-none place-items-center rounded-2xl bg-teal-700 text-sm font-black text-white" aria-hidden="true">V</text>
      <view class="grid min-w-0 flex-1">
        <text class="truncate text-sm font-bold text-slate-950">{{ title }}</text>
        <text class="truncate text-[11px] text-slate-400">{{ subtitle }}</text>
      </view>
      <button class="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-lg text-slate-500" type="button" aria-label="关闭 Agent" @click="emit('close')">×</button>
    </view>
