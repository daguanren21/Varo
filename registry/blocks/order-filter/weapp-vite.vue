...
import VCheckboxGroup from '../ui/v-checkbox-group.vue'
...
<template>
  <view :class="rootClass" aria-labelledby="order-filter-title">
    <view class="mb-5 flex items-center justify-between gap-3">
      <view>
        <text class="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">Orders</text>
        <text id="order-filter-title" class="mt-1 block text-xl font-bold text-slate-950">{{ title }}</text>
      </view>
    ...
    <view class="grid gap-5">
      <view>
        <text class="mb-3 block text-sm font-bold text-slate-800">订单状态</text>
      ...
      <view>
        <text class="mb-3 block text-sm font-bold text-slate-800">订单金额</text>
        <view class="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
          ...
          <text class="pt-5 text-slate-400">—</text>
          <view class="grid gap-1.5 text-xs text-slate-500">
            <text>最高金额</text>
            <VInputNumber v-model:value="maxPrice" :min="0" :max="9999" :step="50" />
          </view>
        </view>
        <text v-if="invalidRange" class="mt-2 block text-sm text-red-600" role="alert">最低金额不能高于最高金额</text>
      </view>
    </view>
