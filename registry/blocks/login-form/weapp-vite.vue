...
<template>
  <view :class="rootClass" aria-labelledby="login-title">
    <view class="mb-6 grid gap-1.5">
      <text class="text-xs font-semibold uppercase tracking-[0.16em] text-teal-700">Varo Account</text>
      <text id="login-title" class="text-2xl font-bold tracking-tight text-slate-950">{{ title }}</text>
      <text class="text-sm leading-6 text-slate-500">{{ description }}</text>
    </view>
    ...
    <form class="grid gap-4" @submit="submit">
      ...
      <text v-if="error" class="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700" role="alert">
        {{ error }}
      </text>

      <view class="flex items-center justify-between gap-4 text-sm">
        <view class="flex items-center gap-2 text-slate-600">
          <VSwitch v-model="remember" />
          <text>记住我</text>
        </view>
        <button class="m-0 bg-transparent p-0 font-semibold text-teal-700" type="button" @click="emit('forgotPassword')">
          忘记密码？
        </button>
      </view>
